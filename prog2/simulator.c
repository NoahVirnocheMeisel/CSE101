...
Bus * makeABus(char *name,int business_seats,int economy_seats,int current_stop_index,char *stops) {
...
for (int i = 0; i < numberBuses;i++) {
    ...
    ListOfBus[i] = makeABus(name,business_seats,economy_seats,current_stop_index,stops);
}
...
Person make_person_from_string(char*string_info) {
    ...
    //this is a very important thing to remember, index is 1 for all the People.
    ...
    stops[0] = 1; //HERE!!
    for (int i = 0; i < strlen(stops_with_commas)+1; i++ ) {
